...
**Type:** `Object | false` **Default:** `false`

**Input types affected:** `select-one`, `select-multiple`

**Usage:** Enable remote data loading with infinite scroll support. When enabled, Choices will fetch data from a remote API with debounced search, pagination, and caching.
...
const example = new Choices(element, {
  remote: {
    // URL template with placeholders or a function
    url: 'https://api.example.com/items?q={query}&page={page}&pageSize={pageSize}',
    // OR
    url: async (query, page, pageSize) => {
    ...
    // Automatically load initial page when dropdown opens
    autoLoadInitial: true,
    
    // Map server response to expected format
    mapResponse: (response) => ({
      items: response.data,      // Array of choices
      page: response.page,        // Current page number
      totalPages: response.total  // Total number of pages
    }),
    
    // Enable request cancellation (using AbortController)
    abortable: true,
...
**URL Options:**
- **String template**: Use placeholders `{query}`, `{page}`, and `{pageSize}` which will be replaced with actual values
- **Function**: Return a Promise with either a Response object or the mapped data directly

**MapResponse:** The function should return an object with:
- `items`: Array of choice objects with `value` and `label` (and optionally other properties)
- `page`: Current page number
- `totalPages`: Total number of pages available

**Features:**
- **Debounced search**: Reduces API calls while typing
- **Abortable requests**: Cancels previous requests when a new one starts
- **Page caching**: Stores previously loaded pages (up to `cachePages` limit)
- **Infinite scroll**: Uses IntersectionObserver sentinels to automatically load previous/next pages
- **Top sentinel**: Prepends previous page when scrolling to top
- **Bottom sentinel**: Appends next page when scrolling to bottom

**See also:** [Remote data example](https://choices-js.github.io/Choices/test/remote/)
...
**Usage:** Clears the remote data cache. Only available when `remote` option is enabled.
...
**Usage:** Refreshes the remote data for the current query by re-fetching from the initial page. Only available when `remote` option is enabled.

**Example:**
```js
choices.refreshRemote();
```

### gotoRemotePage(pageNumber: number);

**Input types affected:** `select-one`, `select-multiple`

**Usage:** Loads a specific page from the remote API. Only available when `remote` option is enabled.

**Example:**
```js
choices.gotoRemotePage(2); // Load page 2
```
...
**Usage:** Returns the current state of the remote data loader including the current query, page, total pages, and cached pages. Returns `null` if remote is not enabled.
...
```ts
{
  ...
  cachedPages: number[];   // Array of cached page numbers for current query
}
```

**Example:**
```js
const state = choices.getRemoteState();
console.log('Current page:', state.currentPage);
console.log('Total pages:', state.totalPages);
console.log('Cached pages:', state.cachedPages);
```
